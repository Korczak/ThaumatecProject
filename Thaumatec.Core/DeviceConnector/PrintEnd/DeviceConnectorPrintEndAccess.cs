...
{
    public class DeviceConnectorPrintEndAccess
    {
        public async Task EndPrint(string serialNumber, DeviceConnectorPrintEndRequest request)
        {
                ...
                try
                {
                    ...
                    await handler.db.Prints.UpdateOneAsync(
                        ...
                        new UpdateDefinitionBuilder<Prints>()
                            .Set(x => x.Status, Device.Constants.PrintStatus.Done)
                            .Set(x => x.StoppedTime, request.PrintTime));
